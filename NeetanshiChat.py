...
class Chatbot:
    def __init__(self, api_key, db_handler, logger):
        ...
        self.context = {}  # Maintain context for ongoing conversation
        self.resources_shared = False  # Track if resources have been shared
    ...
    def generate_response(self, user_input, user_info):
        ...
        # Customize response based on user info and recognized entities
        ...
        # Avoid repeating resources information
        if not self.resources_shared:
            self.resources_shared = True
            response_content += "\n\nRemember, there are resources available to support you, such as counseling services and helplines. If you need additional guidance or just someone to talk to, don't hesitate to reach out."
    ...
    def recognize_intent(self, input_text):
        lemmatized_text = self.lemmatize_text(input_text.lower())
        stress_keywords = ["stress", "anxious", "anxiety", "pressure", "tense", "overwhelmed", "worry", "worried", "nervous", "panic", "lonely", "isolation", "isolated"]
    ...
    def update_context(self, user_input, user_info):
    ...
    def handle_follow_up(self, user_input):
        ...
        if last_input:
            combined_input = f"{last_input} {user_input}"
...
def main():
    ...
    def handle_user_input(session, user_input, related_questions, user_info):
        ...
        if user_input in scenarios:
        ...
        elif chatbot.last_question and chatbot.last_question in scenarios and user_input in scenarios[chatbot.last_question]:
            chatbot.last_question = user_input  # Track the last question
            response = chatbot.generate_response(user_input, user_info)
            print("Neetanshi:", response)
            ...
            user_input = chatbot.handle_follow_up(user_input)
            if chatbot.is_mental_health_related(user_input):
                response = chatbot.generate_response(user_input, user_info)
                print("Neetanshi:", response)
                chatbot.last_question = user_input  # Track the last question
                chatbot.update_context(user_input, user_info)
                related_questions = scenarios.get(chatbot.last_question, {}).get(user_input, {})
            else:
        ...
        while True:
            ...
            if next_input.lower() in ['exit', 'quit', 'e', 'q', '1']:
                ...
                user_input = chatbot.handle_follow_up(next_input)
                if chatbot.is_mental_health_related(user_input):
                    response = chatbot.generate_response(user_input, user_info)
                    print("Neetanshi:", response)
                    chatbot.last_question = next_input  # Track the last question
                    chatbot.update_context(user_input, user_info)
                    return handle_user_input(session, next_input, related_questions, user_info)
                else:
                    response = "Sorry, I am here for mental health-related topics. Do you have any questions or issues related to mental health?"
                    print("Neetanshi:", response)
                    logger.log_interaction(next_input, response)
